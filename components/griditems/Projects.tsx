  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-3 left-4 flex gap-2">
        ...
        {isHovered && <button className="">to the IncExp website</button>}
      </div>
    </div>
  );
};

export const BlogTdd = () => {
  const [isHovered, setIsHovered] = useState(false);
        ...
        <div className={` ${isHovered} ? "scale-x-100" : "" `}>
          ...
          >
            <MdArrowOutward
              ...
              onMouseDown={(e) => {
                e.stopPropagation();
                window.open("https://blog-tdd.vercel.app/");
              }}
            />
          </motion.button>
        </div>
        {isHovered && <button className="">to the Blog website</button>}
      </div>
    </div>
  );
};
